...
function codepenPrefill(code, codeExample) {
  code.each(function() {
    ...
    if (type == "html") {
        ...
        var reAlert = new RegExp(imgAlert,"g");
        HTML = HTML.replace(reStart, 'http://placehold.it/').replace(reEnd, '').replace(reSuccess, '/e1faea').replace(reWarning, '/fff3d9').replace(reAlert, '/f7e4e1');
    } else if (type == "css") {
        CSS = CSS + el.html();
    } else if (type == "js") {
        JS = JS + el.html();
    ...
    var data = {
      ...
      js_library         : "",
      html_classes       : "",
      css_external       : "https://stackpath.bootstrapcdn.com/bootstrap/5.0.0-alpha1/css/bootstrap.min.css",
      js_external        : "https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js;https://stackpath.bootstrapcdn.com/bootstrap/5.0.0-alpha1/js/bootstrap.min.js",
      template           : true
    };
